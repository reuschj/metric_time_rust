...
//! - ⏱️ `TimeConversions`: Provides time unit definitions for both metric and standard time
//!
//! # 📝 Examples
//!
//! ```
//! use metric_time::{Converter, TimeConversions};
//!
...
/// ```
///
/// 🛠️ You can also create a custom converter with a specific conversion rate:
///
/// ```
/// use metric_time::Converter;
///
...
pub struct Converter {
    rate: f64, // 🧮 Conversion rate between time systems
}

impl Converter {
    /// 🆕 Creates a new converter with the specified rate
    pub fn new(rate: f64) -> Self {
        Self { rate }
    }

    /// 🔟 Creates a converter for metric time
    pub fn metric() -> Self {
    ...
    /// 🔄 Converts from origin system to destination system
    ...
    /// 🔄 Converts from destination system back to origin system
...
/// ⏱️ Represents time unit conversions for a specific time system.
...
///
/// ```
/// use metric_time::TimeConversions;
///
/// // Get standard time definitions
/// let std_time = TimeConversions::standard();
/// assert_eq!(std_time.hours_per_day(), 24);
...
/// ```
///
/// 🛠️ You can also create custom time unit definitions:
///
/// ```
/// use metric_time::TimeConversions;
///
...
pub struct TimeConversions {
    hr_per_day: u8,  // 🕐 Hours per day
    min_per_hr: u8,  // ⏱️ Minutes per hour
    sec_per_min: u8, // ⏲️ Seconds per minute
}

impl TimeConversions {
    /// 🆕 Creates a new TimeConversions with custom time unit definitions
    ...
    /// 🕒 Creates standard time conversions (24 hours, 60 minutes, 60 seconds)
    pub fn standard() -> Self {
        TimeConversions::new(24, 60, 60)
    }

    /// 🔟 Creates metric time conversions (10 hours, 100 minutes, 100 seconds)
    pub fn metric() -> Self {
