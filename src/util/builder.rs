...
/// 🛠️ A builder pattern for handling numeric calculations.
///
...
pub struct Builder<T> {
    pub current: T,       // 🔍 Current value being built
    pub previous: Vec<T>, // 📜 History of previous values
}
...
impl<T: Num + Copy> Builder<T> {
    ...
    /// ✖️ Multiplies the current value by another value.
    ///
    /// # 📥 Arguments
    ///
    ...
    /// use metric_time::Builder;
    ///
    /// let result = Builder::new(10.0).multiply_by(2.0).build(); // 20.0
    /// ```
    pub fn multiply_by(mut self, amount: T) -> Self {
        let current = self.current;
        self.previous.push(current);
        self.current = current * amount;
        self
    ...
    /// ✖️ Multiplies by the current value of another Builder.
    ///
    /// # 📥 Arguments
    ///
    ...
    /// let result = builder1.multiply_by_builder(builder2).build(); // 20.0
    /// ```
    pub fn multiply_by_builder(self, builder: Builder<T>) -> Self {
        self.multiply_by(builder.build())
    }
